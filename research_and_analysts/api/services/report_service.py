...
class ReportService:
    ...
    def start_report_generation(self, topic: str, max_analysts: int):
        ...
        try:
            ...
            for event in self.graph.stream({"topic": topic, "max_analysts": max_analysts}, thread, stream_mode="values"):
    ...
    def submit_feedback(self, thread_id: str, feedback: str):
        ...
        try:
            thread = {"configurable": {"thread_id": thread_id}}
            self.logger.info("Submitting feedback for thread_id", thread_id=thread_id,feedback=feedback)
