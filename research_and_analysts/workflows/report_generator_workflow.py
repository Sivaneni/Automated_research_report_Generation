...
class AutonomousReportGenerator:
    ...
    def __init__(self, llm):
        ...
        self.tavily_search = TavilySearchResults(
            tavily_api_key="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"
        )
    ...
    def build_graph(self):
        ...
        try:
            ...
            def initiate_all_interviews(state: ResearchGraphState):
                #print ResearchGraphState
                
                topic = state.get("topic", "Untitled Topic")
                self.logger.info("Initiating interviews for all analysts", topic=topic)
                analysts = state.get("analysts", [])
                self.logger.info("Number of analysts found", count=len(analysts))
                if not analysts:
